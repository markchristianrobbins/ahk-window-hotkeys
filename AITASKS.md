...
- [x] Detached Testing Logs & State: Moved all interactive Command and Keyboard walkthrough verification results (Logs and index state tracking) from main configurations (`HotWinAHK.ini`) to an independent, dedicated test database `tests.ini`, preventing config clutter and organizing test footprints. ✅ 2026-06-22
- [x] Interactive Command Verification Test Dialog: Programmed `CommandTest` walkthrough which applies each action to the window active when the test started, presenting a polished dark-themed rating UI (Yes, No, Unsure, Skip, etc.), logging results directly in `tests.ini` under `[CommandTestLogs]`, and supporting seamless resuming using `[CommandTestState]`.
